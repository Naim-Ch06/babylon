...
function startGame() {
  ...
  let hero = scene.getMeshByName("heroTank");

  scene.toRender = () => {
    let deltaTime = engine.getDeltaTime(); // remind you something ?

    hero.move();
    hero.fireCannonBalls(); // will fire only if space is pressed !
    hero.fireLasers(); // will fire only if l is pressed !
...
function createScene() {
  ...
  //console.log("zafazf :");
  let hero = createTank(scene);
  // second parameter is the target to follow
  scene.followCameraTank = createFollowCamera(scene, hero);
...
function loadSounds(scene) {
  ...
  binaryTask = assetsManager.addBinaryFileTask("pirates", "sounds/battle.mp3");
  binaryTask.onSuccess = function (task) {
...
function createGround(scene) {
  ...
  const ground = BABYLON.MeshBuilder.CreateGroundFromHeightMap("ground","textures/heightMapTriPlanar.png",groundOptions,scene);
  ...
  function onGroundCreated() {
    ...
    terrainMaterial.diffuseTexture = new BABYLON.Texture("textures/grass.png");
...
function createTank(scene) {
  ...
  tank.move = () => {
    ...
    let pickInfo = scene.pickWithRay(ray, (mesh) => { return (mesh.name === "ground"); });
...
function moveHeroDude() {
  let heroDude = scene.getMeshByName("heroTank");
